...
window.processRestoredAttack = function (event) {
    ...
    const restoredMsg = {
        ...
        cyanide_hostname: event.cyanide_hostname || event.honeypot || 'honeypot',
...
function restoreMarkerData(restoredMsg, srcLatLng, dstLatLng, originalEvent) {
    ...
    if (!markerAttackData[dstKey]) {
        markerAttackData[dstKey] = {
            ...
            hostname: restoredMsg.cyanide_hostname,
            attacks: [],
            totalAttacks: 0,
    ...
    addMarker(restoredMsg.dst_country_name, restoredMsg.dst_iso_code,
        restoredMsg.dst_ip, restoredMsg.cyanide_hostname, dstLatLng);
}
...
function addMarker(dst_country_name, dst_iso_code, dst_ip, cyanide_hostname, dstLatLng) {
    // Validate parameters
...
const messageHandlers = {
    Traffic: (msg) => {
        ...
        Promise.all([
            ...
            addMarker(msg.dst_country_name, msg.dst_iso_code, msg.dst_ip, msg.cyanide_hostname, dstLatLng),
        ...
        if (window.attackMapDashboard) {
            const attackData = {
                ...
                src_ip: msg.src_ip,
                ip_rep: msg.ip_rep,
                cyanide_hostname: msg.cyanide_hostname,
                color: msg.color,
                country: msg.country,
                country_code: msg.iso_code,
                iso_code: msg.iso_code,
                protocol: msg.protocol,
                honeypot: msg.honeypot, // Use honeypot field from message, not cyanide_hostname
                port: msg.dst_port,
